...
function StatCard({
  ...
  const inner = (
    <div className="group flex h-full min-w-0 items-center gap-3 rounded-2xl border border-border bg-card p-3 shadow-sm transition-all duration-200 hover:border-border/80 hover:shadow-md sm:gap-4 sm:p-4">
      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl sm:h-12 sm:w-12 ${iconBg}`}>
        <Icon className={`h-4 w-4 sm:h-5 sm:w-5 ${iconColor}`} />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-xl font-bold tabular-nums text-foreground sm:text-2xl">{value}</p>
        <p className="text-[11px] leading-snug text-muted-foreground line-clamp-2 sm:text-xs">{label}</p>
        {sub && <p className="mt-0.5 text-[11px] font-medium text-emerald-600 dark:text-emerald-400">{sub}</p>}
      </div>
      {href && <ChevronRight className="hidden h-4 w-4 shrink-0 text-muted-foreground/40 transition-transform group-hover:translate-x-0.5 sm:block" />}
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-6xl space-y-6 px-4 py-6 lg:px-8">
        ...
        {reachedFreeJobLimit && (
          <div className="flex flex-col gap-3 rounded-2xl border border-amber-400/70 bg-amber-50 px-4 py-3 text-xs text-amber-900 sm:flex-row sm:items-start sm:justify-between dark:border-amber-500/60 dark:bg-amber-950/40 dark:text-amber-100">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">
              {stats?.companyName ? (
                <>
                  Welcome back,{" "}
                  <span className="break-words text-muted-foreground">{stats.companyName}</span>
                </>
              ) : (
                "Dashboard"
              )}
          ...
          <div className="flex w-full min-w-0 flex-col gap-2 sm:w-auto sm:max-w-md sm:flex-row sm:items-center">
            <div className="relative w-full min-w-0 sm:w-64">
              ...
              <Input
                placeholder="Search demands & candidates…"
                className="h-10 rounded-xl pl-9"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </div>
            <Button asChild className="inline-flex h-10 w-full shrink-0 items-center justify-center gap-1.5 rounded-xl sm:w-auto">
                ...
                <div className="space-y-1.5">
                  {filteredSubmissions.map(s => (
                    <Link key={s.id} href={`/company/demands/${s.demandId}`}
                      className="group flex min-w-0 max-w-full items-center gap-2 rounded-xl border border-transparent p-3 transition-all hover:border-border hover:bg-muted/40 sm:gap-3">
                      <Avatar name={s.candidateName} />
                      <div className="min-w-0 flex-1 overflow-hidden">
                        <p className="break-words text-[14px] font-semibold leading-tight text-foreground line-clamp-2 sm:truncate sm:line-clamp-none">
                          {s.candidateName}
                        </p>
                        <p className="mt-0.5 break-words text-[11px] text-muted-foreground line-clamp-2 sm:truncate sm:line-clamp-none">
                          {s.candidateRole || s.demandTitle}
                        </p>
                      </div>
                      <div className="flex shrink-0 flex-col items-end gap-1 sm:flex-row sm:items-center sm:gap-2">
                        <StatusPill status={s.status} />
                        <ChevronRight className="hidden h-3.5 w-3.5 text-muted-foreground/30 transition-transform group-hover:translate-x-0.5 sm:block" />
                      </div>
                    </Link>
                  ))}
                </div>
